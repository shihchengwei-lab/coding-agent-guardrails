...
（Failure mode、Rollback、Independent check command）。把
...
- **high 獨立檢查**：`Independent check command:` 必須存在、不同於 Stop 主檢查，
  並在每次 Stop 執行；失敗、timeout 或命令不存在都 block。
...
`Failure mode:`、`Rollback:`、`Independent check command:`。舊 artifact 不含 Rigor 時保持
legacy 相容。
