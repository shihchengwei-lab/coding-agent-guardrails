...
5. **先找反證。** 每條候選路徑都先寫出「看到什麼就證明這條路錯」，用最便宜的 probe 主動嘗試推翻它。
6. **沒有 evidence 的路徑萎縮。** 猜測型、重構型、架構型分支若無必要證據，就剪掉。
7. **剪掉的路要記錄。** 被否決的設計不能在下一輪 agentic loop 裡無聲復活。
8. **達到停止條件就停。** 不因為 AI 還能美化、抽象、補 fallback，就繼續修改。

走廊 artifact 的負擔分三級：`trivial` 要求 Outcome、Paths、Stop Condition；
`normal` 是預設，再加入 Supports／Would falsify；`high` 再加入 Failure mode、
Rollback 與 Independent check。Goal / Start Frontier 仍是找路方法，不必把每一步
推理重複抄進執行契約。既有未寫 `Rigor` 的走廊在 product delta 前可讀取，
但收工前必須完成 migration。
...
coding 裡的最短路徑應該是 **最小充分語義位移**：先完整滿足目標，
再在充分方案中選額外假設、總複雜度與維護負擔最低者。較少 LOC
若增加隱藏狀態或理解成本，不算更簡單。
...
工具鏈實際解析的欄位以 [`../templates/.slime/corridor.md`](../templates/.slime/corridor.md) 為準。未寫 `Rigor` 的 corridor 在出現 product delta 後必須遷移；明確選 tier 時，hooks 會驗證該 tier 的必要 sections 與固定 labels。
...
## Stop Condition
- Check: <trusted check ID>
```

若保留新 dependency，另在 Evidence 加
`- Dependency: <package> — <why required>`；high tier 再加入
`## High-risk Controls`，並用 `Independent check:` 指定不同於 primary checks 的
secondary trusted check ID。argv 只由 `<git-dir>/guardrails/config.json` 提供。
...
```text
[ ] Food Points 已列出。
[ ] Rigor 已依風險選為 trivial、normal 或 high。
[ ] Goal / Start Frontier 已在探索中對齊，不必重抄到 artifact。
[ ] normal／high 已列出 Supports 與 Would falsify。
[ ] high 已列出 Failure mode、Rollback 與 Independent check。
[ ] Outcome 與 Paths 已把最小充分走廊寫清楚。
[ ] Stop Condition 已列出。
```
