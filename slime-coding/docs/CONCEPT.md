...
走廊負擔分三級：`trivial` 只要求 Scope、Paths、Stop Condition；
`normal` 是預設，加入雙向 frontier、Semantic Delta、Non-goals，以及
Supports／Would falsify；`high` 再加入 Failure mode、Rollback 與
Independent check。既有未寫 `Rigor` 的走廊維持 legacy 相容。
...
工具鏈實際解析的欄位以 [`../templates/.slime/corridor.md`](../templates/.slime/corridor.md) 為準。未寫 `Rigor` 時只沿用 legacy 的 header／Paths 檢查；明確選 tier 時，hooks 會驗證該 tier 的必要 sections 與固定 evidence labels。本節格式是概念層示意。
