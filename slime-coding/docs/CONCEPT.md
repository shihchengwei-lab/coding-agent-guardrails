...
推理重複抄進執行契約。既有未寫 `Rigor` 的走廊維持 legacy 相容。
...
工具鏈實際解析的欄位以 [`../templates/.slime/corridor.md`](../templates/.slime/corridor.md) 為準。未寫 `Rigor` 時只沿用 legacy 的 header／Paths section 形狀，但 match-all Paths 仍會拒絕；明確選 tier 時，hooks 會驗證該 tier 的必要 sections 與固定 labels。
