...
- **負擔隨風險調整**：trivial 只留最小三項；normal 是預設；high 再加入 failure mode、rollback 與 independent check。
...
需要 `python3` 和 `git`。新安裝的 corridor 預設為 `normal`。安裝可以重跑，會備份既有設定，也不會覆蓋已存在的 corridor。
...
- 它不能判斷所有設計好壞，只能擋幾個明確事實：格式不完整、範圍外檔案、新套件、紅燈收工、型別檢查失敗。Rigor 是否低報只提示，不自動替作者升級。
