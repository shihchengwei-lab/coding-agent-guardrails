...
- **動手前先框範圍**：AI 要先寫出可觀察結果與允許路徑；寫入範圍外檔案會在工具執行前被拒絕。
- **全 repo 與 repo 外都不是走廊**：match-all Paths 和 `../`／絕對路徑會被拒絕。
- **先找反證再押路線**：normal／high 走廊除了支持證據，也要寫出什麼觀察會證明這條路錯了。
- **負擔隨風險調整**：trivial 只留 Outcome／Paths／Stop Condition；normal 加支持與反證；high 再加入 failure mode、rollback 與 independent check。
...
- **紅燈不能假裝完成**：`SLIME_TEST_CMD` 或 Stop Condition 的 `Command:` 會在每次 Stop 重跑；紅燈、timeout 或設定錯誤都持續阻擋。PRUNED 不能當 waiver。
